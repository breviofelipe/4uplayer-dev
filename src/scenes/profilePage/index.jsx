...
const ProfilePage = () => {
  ...
  const getStatsGamer = async () => {
    setLoading(true);
    const responseStats = await fetch(url+`/users/stats/${userId}/fortnite`, {
      method: "GET",
      headers: { Authorization: `Bearer ${token}` },
    });
    if(responseStats.status === 200 ){
      const datastats = await responseStats.json();
      setPlayerData(datastats);
    }
    setLoading(false);
  };
  ...
  const top = () => {
    ...
    {!isLoading && playerData && <StatsWidget data={playerData.data} />}
    </>
  }
