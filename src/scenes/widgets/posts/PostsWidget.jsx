...
const PostsWidget = ({ userId, isProfile = false }) => {
  ...
  //   setLoading(true);
  //   const response = await fetch(urlEnv+"/posts?page=0&sizePerPage=10&sortDirection=DESC", {
  //     method: "GET",
  //     headers: { Authorization: `Bearer ${token}` },
  //   });
  //   const data = await response.json();
  //   dispatch(setPosts({ posts: data.content }));
  //   setLoading(false);
  // };
  ...
  return <>{isLoading ? <GamerLoading /> : <><InfiniteScroll /></> }</>;
};
